...
class Branch_operating(object):
    ...
    def add_branch(self, branch_name, branch_city, branch_assets):
        sql = r"INSERT INTO branch(branch_name, branch_city, branch_assets) VALUES({}, {}, {});"\
            .format(branch_name, branch_city, branch_assets)
        self.__execute_sql(sql)

    # 删除一个支行的所有信息
    def delete_branch(self, branch_name):
        sql = r"DELETE FROM branch WHERE branch_name={};"\
            .format(branch_name)
        self.__execute_sql(sql)
...
def branch_operating(request):
    return render(request, template_name='branch_management_system/branch_UI.html')
